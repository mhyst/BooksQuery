...
import julk.net.deliver.ESMTPTranslator;
				...
				for (int l = 0; l < msg.linesCount(); l++) {
					...
					if (DELIVERER.add(wi)) 
						...
						FileOutputStream fout = new FileOutputStream ("error.mme");
                                                fout.write(ESMTPTranslator.getDynamicHeader(from).getBytes());
                                                fout.write(ESMTPTranslator.before.getBytes());
						fout.write("Subject: Robomail error\r\n".getBytes());
                                                fout.write(ESMTPTranslator.after.getBytes());
                                                fout.write("\r\n".getBytes());
